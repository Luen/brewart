...
export async function getIngredients() {
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const productUrl = $(element).find('h4 a').attr('href').trim();
      ...
      const longDescription = await getLongDescription(productUrl);

      products.push({ id, productUrl, name, image, shortDescription, longDescription, price });
    }
