...
async function getLongDescription(url) {
  try {
    ...
    const longDescription = $('.product.attribute.description').text().trim().replace(/<br>|\n|\r/g, '/\n').replace(/&nbsp;/g, ' ').replace(/\s{2,}/g, ' ');
...
export async function getIngredients() {
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const countryOfOrigin = productOrigins[id] || "Unknown";

      const longDescription = await getLongDescription(url);

      products.push({ id, url, name, image, shortDescription, longDescription, price, countryOfOrigin });
    }
    ...
    products.push({ id: 'C', url: droidCleanseUrl, name, image, shortDescription, longDescription, price });
