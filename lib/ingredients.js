...
export async function getIngredients() {
  ...
  try {
    ...
    const shortDescription = $droidCleanse('#brewInfo div.product.attribute.overview').text().trim();
    const longDescription = $droidCleanse('.product.attribute.description').text().trim();
    const priceText = $droidCleanse('.price-box.product-info-price .price').text().trim().replace('$', '');
    const price = parseFloat(priceText);
    const backgroundImageStyle = $droidCleanse('.brewHero').css('background-image');
    let image = '';
    ...
    products.push({ id: 'C', url: droidCleanseUrl, name: 'Droid Cleanse', image, shortDescription, longDescription, price });
