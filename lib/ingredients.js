import * as cheerio from 'cheerio'
import ingredientsInfo from './ingredientsInfo.json' with { type: 'json' }
import productOrigins from './productOrigins.json' with { type: 'json' }

async function getLongDescription(url) {
    try {
        const response = await fetch(url, {
            next: { revalidate: 604800 }, // one week
        })
        if (!response.ok) {
            console.error(`Failed to fetch long description from ${url}`)
            return '' // Return empty string as fallback
        }
        const html = await response.text()
        const $ = cheerio.load(html)
        const longDescription = $('#brewInfo > .product.attribute.description')
            .text()
            .trim()
            .replace(/<br>|\n|\r/g, '\n')
            .replace(/&nbsp;/g, ' ')
            .replace(/\s{2,}/g, ' ')
        return longDescription
    } catch (error) {
        console.error(`Error fetching long description from ${url}:`, error)
        return '' // return an empty string as fallback
    }
}

export async function getIngredients() {
    const url = 'https://brewart.com/au/store/ingredients'

    try {
        // Try to fetch ingredients page
        const response = await fetch(url, {
            next: { revalidate: 604800 }, // one week
        })
        if (!response.ok) {
            console.error(
                'Failed to fetch ingredients list, using fallback data'
            )
            return [] // Return empty array as fallback
        }

        const html = await response.text()
        const $ = cheerio.load(html)

        let products = []
        const items = $('.brew-prod-item')

        // Process each ingredient with error handling
        for (let i = 0; i < items.length; i++) {
            try {
                const element = items[i]
                const url = $(element).find('h4 a').attr('href')?.trim()
                const image = $(element)
                    .find('.product-image-photo')
                    .attr('src')

                if (!url || !image) {
                    console.error('Missing required data for ingredient', i)
                    continue
                }

                // Extract ID from image filename
                let id = image.split('/').pop().split('.')[0]
                id = id.split('_')[0].toUpperCase()

                if (id === 'ICON') {
                    const iconNumber = image
                        .split('/')
                        .pop()
                        .split('.')[0]
                        .split('_')[2]
                    id = iconNumber.toUpperCase()
                } else if (id === 'PRIMER') {
                    id = 'P'
                }

                const name = $(element).find('h4 a').text().trim()
                const shortDescription = $(element)
                    .find('.short-desc p')
                    .text()
                    .trim()
                const priceText = $(element)
                    .find('.price-box .regular-price .price')
                    .first()
                    .text()
                    .trim()
                    .replace('A$', '')
                    .replace(/\$/g, '')
                const price = parseFloat(priceText)

                if (!name) {
                    console.error('Missing name for ingredient', i)
                    continue
                }

                if (isNaN(price)) {
                    console.error(
                        'Invalid price for ingredient',
                        name,
                        priceText
                    )
                    continue
                }

                const countryOfOrigin = productOrigins[id] || ''

                let category
                switch (id[0]) {
                    case 'C':
                        category = 'Droid Cleanse'
                        break
                    case 'Y':
                        category = 'Yeast'
                        break
                    case 'E':
                        category = 'Elements'
                        break
                    case 'X':
                        category = 'Enhancers'
                        break
                    case 'H':
                        category = 'Hop Oils'
                        break
                    case 'D':
                        category = 'Dry Hops'
                        break
                    case 'P':
                        category = 'Primer'
                        break
                    default:
                        category = null
                }

                const info =
                    category && id[0] !== 'P' && id[0] !== 'C'
                        ? ingredientsInfo[category]?.[id] || {}
                        : {}
                const longDescription = await getLongDescription(url)

                products.push({
                    id,
                    category,
                    url,
                    name,
                    image,
                    shortDescription,
                    longDescription,
                    price,
                    countryOfOrigin,
                    info,
                })
            } catch (error) {
                console.error('Error processing ingredient:', error)
                continue
            }
        }

        // Try to fetch Droid Cleanse data
        try {
            const droidCleanseUrl = 'https://brewart.com/au/droid-cleanse'
            const droidCleanseResponse = await fetch(droidCleanseUrl, {
                next: { revalidate: 604800 },
            })

            if (droidCleanseResponse.ok) {
                const droidCleanseHtml = await droidCleanseResponse.text()
                const $droidCleanse = cheerio.load(droidCleanseHtml)

                // Extract name from the page title or heading
                const name =
                    $droidCleanse('h1, .page-title, .product-name')
                        .first()
                        .text()
                        .trim() || 'Droid Cleanse'

                // Extract description from the page content
                const longDescription = $droidCleanse(
                    'p, .description, .product-description'
                )
                    .map((i, el) => $droidCleanse(el).text().trim())
                    .get()
                    .filter((text) => text.length > 20) // Filter out short text
                    .join('\n\n')
                    .replace(/<br>|\n|\r/g, '\n')
                    .replace(/&nbsp;/g, ' ')
                    .replace(/\s{2,}/g, ' ')

                // Extract price - try multiple selectors
                const priceText = $droidCleanse(
                    '.price, .product-price, [class*="price"]'
                )
                    .first()
                    .text()
                    .trim()
                    .replace('A$', '')
                    .replace(/\$/g, '')
                const price = parseFloat(priceText)

                // Extract image - try multiple selectors
                const image = $droidCleanse(
                    'img[src*="droid"], img[src*="cleanse"], .product-image img, #image'
                )
                    .first()
                    .attr('src')

                if (name && !isNaN(price)) {
                    products.push({
                        id: 'C',
                        category: 'Droid Cleanse',
                        url: droidCleanseUrl,
                        name,
                        image:
                            image ||
                            'https://brewart.com/media/catalog/product/cache/placeholder.png',
                        shortDescription:
                            'A single-use sanitising tablet used to create a bacteria free environment.',
                        longDescription:
                            longDescription ||
                            'Use one effervescent tablet in your BeerDroid prior to brewing for pure, quality beer. Or use one effervescent tablet in a 5-10L vessel to sanitise kegging equipment prior to use.',
                        price,
                        countryOfOrigin: 'Australia',
                        info: {},
                    })
                }
            }
        } catch (error) {
            console.error('Error fetching Droid Cleanse data:', error)
        }

        // Reorder products by category
        const categoryOrder = [
            'Droid Cleanse',
            'Yeast',
            'Elements',
            'Enhancers',
            'Hop Oils',
            'Dry Hops',
            'Primer',
        ]
        products.sort((a, b) => {
            const orderA = categoryOrder.indexOf(a.category)
            const orderB = categoryOrder.indexOf(b.category)
            return orderA - orderB
        })

        return products
    } catch (error) {
        console.error('Error in getIngredients:', error)
        return [] // Return empty array as fallback
    }
}
