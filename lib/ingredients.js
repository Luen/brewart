...
export async function getIngredients() {
    ...
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (id === 'ICON') {
                const iconNumber = image
                    ...
                    .pop()
                    .split('.')[0]
                    .split('_')[2] // Extract number for ICON
        ...
        const longDescription = $droidCleanse('.product.attribute.description')
            .text()
            .trim()
            .replace(/<br>|\n|\r/g, '\n')
            .replace(/&nbsp;/g, ' ')
            .replace(/\s{2,}/g, ' ')
        const priceText = $droidCleanse('.price-box.product-info-price .price')
            .text()
            .trim()
            .replace('$', '')
        const price = parseFloat(priceText)
        ...
        // Reorder to have Droid Cleanse first, then Yeast, Elements, Enhancers, Hop Oils, Dry Hops, Primer
        const categoryOrder = [
            'Droid Cleanse',
            'Yeast',
            'Elements',
