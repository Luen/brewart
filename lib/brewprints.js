...
async function getBrewprintIngredients(url) {
  try {
    ...
    return { url, name, imageIcon, imageBanner, shortDescription, longDescription, primaryRating, recommended, price, totalPriceOfIngredients, alcoholByVolume, colour, bitterness, brewingProgram, approximateBrewingTime, brewNotes, insideTheBox };
  } catch (error) {
    console.error(error);
  }
}
