...
async function getBrewprintIngredients(url) {
  try {
    ...
    const shortDescription = $brew('#brewInfo div.product.attribute.overview').text().trim().replace("-tsar","-star");
    const longDescription = $brew('.product.attribute.description').text().trim().replace(/\n|\r|&nbsp;|<br>/g, ' ').replace(/\s{2,}/g, ' ');
    ...
    const yeasts = [];
    const primers = [];
    const items = $brew('[class*=bb-element-], [class*=bb-enhancer-], [class*=bb-hops-], [class*=bb-dry-hop-], [class*=bb-yeast-], [class*=bb-primer]');
    items.each((index, element) => {
      const classAttr = $brew(element).attr('class');
      let matches = classAttr.match(/bb-(element|enhancer|hops|dry-hop|yeast|primer)-(\d+)(?:-(\w+))?/i);
    ...
    return { url, name, imageIcon, imageBanner, shortDescription, longDescription, primaryRating, recommended, price, totalPriceOfIngredients, alcoholByVolume, colour, bitterness, brewingProgram, approximateBrewingTime, tastesLike, brewNotes, insideTheBox, countryOfOrigin };
  } catch (error) {
    console.error(error);
  }
}
