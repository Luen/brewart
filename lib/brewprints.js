...
import brewprintsTasteLike from './brewprintsTasteLike.json' with { type: 'json' }
...
async function getBrewprintIngredients(url) {
    try {
        ...
        // Try multiple price selectors to handle different page structures
        let priceText = $brew('.price-box.product-info-price .price')
            .text()
            .trim()

        // If not found, try alternative selectors
        if (!priceText) {
            priceText = $brew('.price-wrapper .price').text().trim()
        }
        if (!priceText) {
            priceText = $brew('[data-price-amount]').attr('data-price-amount')
        }
        if (!priceText) {
            priceText = $brew('.price').first().text().trim()
        }

        // Clean up the price text
        priceText = priceText.replace('A$', '').replace(/\$/g, '')
